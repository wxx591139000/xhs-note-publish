...
- **按用途筛选**（v1.3.0）：桌面「笔记队列」顶部新增「所有 / 📕小红书 / 🐟闲鱼 / 🔀通用」分类，点「所有」列出全部类型，可与上方的状态筛选叠加使用。
...
- 每篇可在 **「闲鱼宝贝信息」** 填价格/成色/发货地等（每行 `键:值`），手机端点 **「🐟 复制宝贝文案」** 一键拼出闲鱼发布文案。
- 每篇可标**用途**：📕小红书 / 🐟闲鱼 / 🔀通用。桌面端显示徽标、手机端可**按平台筛选**、按钮按用途收敛——小红书稿只显示「复制文案」，闲鱼稿只显示「复制宝贝文案」，根除点混。
- 沿用同一理念：闲鱼同样**不做 API 自动发布**，一键复制后人工粘贴最稳。
...
> 隧道配置在 `~/.cloudflared/config.yml`，本地走独立 `xhs-tunnel`(27da88b4) 只管 xhs，与 VPS 的 transcribe-bot 隧道彻底隔离（勿用旧配置重启抢隧道致 502）。
